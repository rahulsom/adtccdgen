...
class GenerateCsv {
  ...
  public static void main(String[] args) {
    ...
    def util = new PersonFactory(maxAddresses: patients / 2000)
    util.setCenter(clinicAddress)

    String nsid = "1005.1"
    String oid = "2.16.4.39.2.1001.78.5.1"

    def f = new File('/tmp/pat.csv').newPrintWriter()
    ...
    patients.times { index ->
      ...
      def cols = [p.firstName,p.lastName,p.dob.format('yyyyMMdd'),p.gender,a.street,a.city,a.state, a.zipCode,"USA",
          p.phone,p.ssn,'B' + new DecimalFormat('00000000').format(index + 10000000),nsid,oid,'ISO']
